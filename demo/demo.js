...
let queuedCameraDelta = { dx: 0, dy: 0, zoom: 0 };
...
function renderState(state) {
  ...
  if (state.ready === false) {
    ...
    elements.loading.querySelector('strong').textContent = state.runtime_error ? 'MuJoCo setup failed' : 'Preparing MuJoCo';
  ...
  if (state.runtime_error) {
    elements.loading.querySelector('strong').textContent = 'Simulation paused';
...
async function sendCommand(command, sourceButton = null) {
  ...
  try {
    previewYawDeg = null;
    renderState(await requestJson('/api/command', {
      method: 'POST',
...
async function flushVelocityTarget() {
  ...
  try {
    renderState(await requestJson('/api/velocity', {
      method: 'POST',
...
async function flushHeightTarget() {
  ...
  try {
    renderState(await requestJson('/api/height', {
      method: 'POST',
...
async function flushYawTarget() {
  ...
  try {
    const state = await requestJson('/api/yaw', {
      method: 'POST',
...
document.querySelectorAll('[data-overlay]').forEach((button) => {
  button.addEventListener('click', async () => {
    try {
      renderState(await requestJson('/api/controller-overlay', {
        method: 'POST',
        body: JSON.stringify({ controller: button.dataset.overlay }),
      }));
    } catch (error) {
      showToast(error.message);
    }
...
elements.cameraFollow.addEventListener('click', async () => {
  try { renderState(await requestJson('/api/camera-follow', { method: 'POST', body: '{}' })); }
  catch (error) { showToast(error.message); }
});

function queueCamera(dx = 0, dy = 0, zoom = 0) {
  queuedCameraDelta.dx += dx;
  queuedCameraDelta.dy += dy;
  queuedCameraDelta.zoom += zoom;
  if (cameraRequestPending) return;
  cameraRequestPending = true;
  window.requestAnimationFrame(async () => {
    const delta = queuedCameraDelta;
    queuedCameraDelta = { dx: 0, dy: 0, zoom: 0 };
    try {
      latestState = await requestJson('/api/camera', { method: 'POST', body: JSON.stringify(delta) });
    } catch (error) {
      showToast(error.message);
    } finally {
      cameraRequestPending = false;
      if (queuedCameraDelta.dx || queuedCameraDelta.dy || queuedCameraDelta.zoom) queueCamera();
    }
  });
}
...
elements.viewport.addEventListener('pointermove', (event) => {
  ...
  queueCamera(event.clientX - cameraDrag.x, event.clientY - cameraDrag.y, 0);
  cameraDrag = { x: event.clientX, y: event.clientY };
});
...
elements.stream.addEventListener('load', () => {
  if (latestState?.ready !== false && !latestState?.runtime_error) {
...
async function pollState() {
  try {
    ...
    elements.connection.querySelector('span').textContent = 'Backend offline';
  } finally {
...
async function initializeDemo() {
  try {
    renderState(await requestJson('/api/default-targets', {
      method: 'POST',
      body: '{}',
    }));
  } catch (error) {
